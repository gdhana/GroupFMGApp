//
//  LabourbagcheckViewController.m
//  Gurgle
//
//  Created by Dhanasekar Gunabalan on 8/30/12.
...
@interface LabourbagcheckViewController ()

@end

@implementation LabourbagcheckViewController
@synthesize btn35lbl;
@synthesize btn34lbl;
@synthesize btn33lbl;
@synthesize btn32lbl;
...
//    [bottombanner startAnimating];
    ...
    [scrollview setContentSize:CGSizeMake(198, 2355)];
    [super viewDidLoad];
	...
	NSString *plistPath = [documentsPath stringByAppendingPathComponent:@"Labourbag.plist"];
	
	// check to see if Data.plist exists in documents
	if (![[NSFileManager defaultManager] fileExistsAtPath:plistPath]) 
	{
		// if not in documents, get property list from main bundle
		plistPath = [[NSBundle mainBundle] pathForResource:@"Labourbag" ofType:@"plist"];
	}
	...
	if (!temp) 
	{
		NSLog(@"Error reading Labourbag plist: %@, format: %d", errorDesc, format);
	}
   	// assign values
...
-(void) Storedata
{
    ...
    {
        NSLog(@"Error in Labourbag saveData: %@", error);
        // [error release];
    }
}
...
- (IBAction)btn1lbl:(id)sender {
    if (checkboxSelected1 == 0){
        ...
        [btn1lbl setTitleColor:[UIColor colorWithRed:44/255.0 green:93/255.0 blue:205/255.0 alpha:1.0] forState:UIControlStateNormal];
       // [btn1lbl setTitleColor:[UIColor blueColor] forState:UIControlStateNormal];
